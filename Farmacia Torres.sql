...
CREATE TRIGGER agregar_farmaco AFTER INSERT ON farmacia
    ...
    /*Crear
      ...
      Alterar
    CREATE USER 'sujetoPrueba@dominioPrueba' IDENTIFIED BY '1234';
    
    Update
    
    UPDATE mysql.user SET Password=PASSWORD('1234')' WHERE user = 'sujetoPrueba' AND host = 'sujetoPrueba';
    
    renombrar
    
    RENAME USER 'sujetoPrueba@dominioPrueba' TO 'sujetoPruebas@dominioPruebas'
    ...
    */ 
    ...
    /* Crea el sujeto de lectura */
CREATE USER 'sujetoLectura@dominioPrueba' IDENTIFIED BY '1234';
    
    SELECT * FROM USER;
    
    SELECT * FROM mysql.user WHERE user LIKE 'sujetoLectura%';
    
    /* Dar y Revocar todos los permisos */
    GRANT ALL ON *.* TO 'sujetoLectura@dominioPrueba';
    REVOKE ALL ON *.* FROM 'sujetoLectura@dominioPrueba';
    /*Se le da el permiso de lectura*/
    GRANT SELECT ON proyecto_alpha.* TO 'sujetoLectura@dominioPrueba';
    
    
    SHOW GRANTS FOR 'sujetoLectura@dominioPrueba';
    
        /* Crea el sujeto con varios permisos */
CREATE USER 'sujetoImportante@dominioPrueba' IDENTIFIED BY '1234';
    
    SELECT * FROM USER;
    
    SELECT * FROM mysql.user WHERE user LIKE 'sujetoImportante%';
    
    /* Dar y Revocar todos los permisos */
    GRANT ALL ON *.* TO 'sujetoImportante@dominioPrueba';
    REVOKE ALL ON *.* FROM 'sujetoImportante@dominioPrueba';
    /*Se le da el permiso de lectura*/
    GRANT SELECT ON proyecto_alpha.* TO 'sujetoImportante@dominioPrueba';
     /*Se le da el permiso de incersion*/
    GRANT INSERT ON proyecto_alpha.* TO 'sujetoImportante@dominioPrueba';
     /*Se le da el permiso de modificacion/actualizacion*/
    GRANT UPDATE ON proyecto_alpha.* TO 'sujetoImportante@dominioPrueba';
    
    SHOW GRANTS FOR 'sujetoImportante@dominioPrueba';
 ...
 INSERT INTO farmacia (id_farmacia, stock_farmaco, nombre_farmaco, precio_farmaco, descripcion_farmaco) VALUES (1, 1, 'Tafirol', 50, 'Pastilla');
...
UPDATE farmacia
 SET descripcion_farmaco = 'Pastilla potente'
 WHERE id_farmacia = 1 AND nombre_farmaco = 'Tafirol';
 ...
 INSERT INTO receta (id_receta, dosis, validez_receta, farmaco_recetado, frecuencia_uso) VALUES 
 (1, 1, 29/03/2022, 'Tafirol','07:09:00'), (2, 5, 30/03/2022, 'Tafirol','07:30:00'), 
(3, 4, 30/03/2022, 'Tafirol','17:30:00'), (4, 1, 05/03/2022, 'Tafirol','10:30:00');

savepoint recetas_1;

 INSERT INTO receta (id_receta, dosis, validez_receta, farmaco_recetado, frecuencia_uso) VALUES  
  (5, 2, 15/03/2022, 'Ibuprofeno','10:00:00'), (6, 1, 15/03/2022, 'Ibuprofeno','12:30:00')
, (7, 1, 25/03/2022, 'Ibuprofeno','10:55:00'), (8, 4, 27/03/2022, 'Ibuprofeno','02:30:00');
...
ROLLBACK TO recetas_1;
 
/*
Creacion de backup, donde se guardan los datos de la tabla Farmacia, y receta
*/

DROP DATABASE datosProject;

CREATE DATABASE datosProject;
/*

COMANDO DE RESTUARACION
mysqldump datosProject < backup.sql
